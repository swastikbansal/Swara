...
class Database:
    ...
    try:
        ...
        if mycon.is_connected():
            messagebox.showinfo(
                "Connection Successfull", "Connected to database Successfully."
            )
    ...
    # If Connection is Unsuccessful
        ...
        messagebox.showerror(
            "Connection Failed",
            "We can't connect to database at this movemnt.\nPlease Try Again Later.",
        )
        quit()
    ...
    def login(self, userEntry, passEntry, func):
        ...
        self.dataRetrieved = (
            self.cur.fetchall()
        )  # Storing username and password from database
    ...
    def regCheck(self, mobileEntry, emailEnrtry, func):
        ...
        # Invalid Input of Mobile No.
            ...
            if self.mobile_no.isdigit() == False:
                messagebox.showerror(
                    "Invalid Input", "Please Enter Valid Mobile No. \nIn Digits"
                )
            ...
            # If Mobile No. satisfies the above conditons
                ...
                for current in self.data:
                    ...
                    if (self.mobile_no in current) and (self.email in current):
                        messagebox.showerror(
                            "Already Exists", "Email and Phone alread exist"
                        )
                        self.cond = False
                    ...
                    # Only Mobile No. Exists
                        ...
                        messagebox.showerror(
                            "Already Exists", "Mobile No. Already Exists"
                        )
                        self.cond = False
        ...
        if (
            ...
            if self.newUser not in self.data:
                ...
                if len(self.newPass) < 6:
                    messagebox.showerror(
                        "Short Password", "Password should have length greater than 6."
                    )
                    ...
                    if self.firstName.isalpha() and self.lastName.isalpha():
                        self.cur.execute(
                            f"INSERT INTO users (Username,F_name,L_name,Mobile_no,Email,Password,DOB) VALUES('{self.newUser}','{self.firstName}','{self.lastName}','{self.newMobile}','{self.newEmail}','{self.newPass}','{self.dob}')"
                        )
                        win.destroy()
                        messagebox.showinfo(
                        ...
                        messagebox.showerror(
                            "Invalid Input",
                            "Please Input First Name and Last Name in Alphabetical form.",
                        )
